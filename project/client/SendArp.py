...
class Node(ctypes.Structure):
    """Structure representing network node information."""
    _fields_ = [('ip', ctypes.c_ulong),  # IP
                ('mac', ctypes.c_ubyte * MAC_LENGTH),  # MAC
...
def get_network_mask(ip):
    ...
    :param ip: ip of the current computer
    :return: network mask
    """
...
def get_ip():
    ...
    :return: ip of the current computer
    """
...
def get_start_ip(ip, mask):
    ...
    :param ip: ip of the current computer
    :param mask: mask of the local network
    :return: start ip of the local network
    """
    iplist = ip.split(".")
    masklist = mask.split(".")
...
def get_end_ip(ip, mask):
    ...
    :param ip: ip of the current computer
    :param mask: mask of the local network
    :return: end ip of the local network
    """
    iplist = ip.split(".")
    masklist = mask.split(".")
...
def get_addresses_dict():
    """
    calls the dll that sends the ARP requests
    :return: returns a dictionary containing the ip and mac address of the online computers in the local network
    """
    ip = get_ip()
