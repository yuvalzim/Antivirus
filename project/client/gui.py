...
class Ui_MainWindow(QtWidgets.QWidget):
    ...
    def setupUi(self, MainWindow, num_of_comps, comp_dict):
        ...
        self.options_button.setGeometry(QtCore.QRect(self.scan_x - 50, self.scan_y, 200, 81))
        font = QtGui.QFont()
        ...
        self.options_button.setIcon(QtGui.QIcon('settings_icon.png'))
        self.options_button.setIconSize(QtCore.QSize(50, 50))
        layout2 = QtWidgets.QVBoxLayout(MainWindow)
        layout2.addWidget(self.options_button)
    ...
    def select_comp(self, button):
        ...
        :return:
        """
        if button.text() == self.ip:
            return
        if self.start_encryption(button.text()) != 0:
            return
    ...
    def start_encryption(self, ip):
        ...
        try:
            self.client_socket.connect((ip, PORT_DST))  # Change to selected IP when tested
        except Exception as e:
            msg = QtWidgets.QMessageBox(self)
            msg.setIcon(QtWidgets.QMessageBox.Critical)
            msg.setText("Operation unsuccessful")
            msg.setInformativeText("error information")
            msg.setWindowTitle("Operation status")
            msg.setDetailedText(f"error details: {e}")
            msg.exec_()
